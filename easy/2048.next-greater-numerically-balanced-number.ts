// https://leetcode.com/problems/next-greater-numerically-balanced-number/solutions/7297218/simple-enumeration/

function nextBeautifulNumber(n: number) {
    ...
    while (true) {
        if ([...count(++n)].every(([k, v]) => k === v)) {
            return n;
        }
    }
}

function nextBeautifulNumber2(n: number) {
    // deno-fmt-ignore
    const lookup = [
    ...
    let l = 0, r = lookup.length - 1;
    while (l < r) {
        const mid = (l + r) >> 1;
        lookup[mid] <= n ? l = mid + 1 : r = mid;
    }
    return lookup[l]; // or return lookup[r];
}

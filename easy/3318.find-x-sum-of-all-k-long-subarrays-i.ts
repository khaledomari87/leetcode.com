...
function findXSum(nums: number[], k: number, x: number) {
    ...
    for (let right = k - 1; right < n; right++) {
        ...
        let subArr = [...counts];
        if (counts.size > x) {
            subArr = subArr.sort((a, b) => b[1] - a[1] || b[0] - a[0]).slice(0, x);
        }
        res[left] = subArr.reduce((s, item) => s + item[0] * item[1], 0);
        const first = counts.get(nums[left])!;
        if (first) counts.set(nums[left], first - 1);
        else counts.delete(nums[left]);
    }
    return res;
}
